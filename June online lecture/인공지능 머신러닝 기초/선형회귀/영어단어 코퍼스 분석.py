...
def main():
    words = read_data()
    #words[:,1] = sorted(words[:,1],reverse = True) # words.txt 단어를 빈도수 순으로 정렬합니다.
    words =np.array(sorted(words, key = itemgetter(1), reverse = True))
    # 정수로 표현된 단어를 X축 리스트에, 각 단어의 빈도수를 Y축 리스트에 저장합니다.  
    X = np.array(list(range(1, len(words)+1))).reshape(-1,1)
